...
export default function RegisterForm() {
  ...
  return (
    <div className="bg-white rounded-sm shadow-md w-full max-w-md p-8">
      <h1 className="text-2xl font-bold text-[#333] mb-1">Crear cuenta</h1>
      <p className="text-sm text-[#666] mb-6">
        ¿Ya tenés cuenta?{' '}
        <Link href="/login" className="text-[#2968C8] hover:underline font-medium">
          Ingresá
        </Link>
      </p>
      ...
      <div className="flex items-center gap-3 my-5">
        ...
        <span className="text-xs text-[#666]">o registrate con</span>
        <div className="flex-1 h-px bg-[#EBEBEB]" />
      </div>
      ...
      <p className="text-xs text-[#666] text-center mt-5">
        Al registrarte aceptás nuestros{' '}
        <span className="text-[#2968C8]">términos y condiciones</span>
      </p>
    </div>
  )
}
...
function mensajeError(e: unknown): string {
  if (e instanceof Error) {
    ...
    if (code === 'auth/popup-closed-by-user') {
      return 'Se canceló el registro con Google'
    }
    return e.message
  }
